...
future<> group0_state_machine::reload_modules(modules_to_reload modules) {
    ...
    if (update_auth_cache_roles.size()) {
        ...
        if (audit::audit::audit_instance().local_is_initialized()) {
            ...
            // it was dropped.  This is safe because load_roles() is
            // co_await-ed above, so the cache is fully up-to-date here.
            std::vector<sstring> created_roles, dropped_roles;
            for (const auto& role : audit_role_names) {
                if (_ss.auth_cache().get(role)) {
                    created_roles.push_back(role);
                } else {
                    dropped_roles.push_back(role);
                }
            }
            co_await audit::audit::audit_instance().invoke_on_all(
                [&created_roles, &dropped_roles] (auto& a) {
                    for (const auto& role : created_roles) {
                        a.on_role_created(role);
                    }
                    for (const auto& role : dropped_roles) {
