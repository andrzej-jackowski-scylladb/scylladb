...
future<> inspect(shared_ptr<cql3::cql_statement> statement, service::query_state& query_state, const cql3::query_options& options, bool error) {
    ...
    // Error injection: pause here so a test can trigger server shutdown
    // while a CQL query is in-flight inside inspect(), reproducing the
    // race where stop_audit() destroys the audit service before the
    // maintenance CQL server is drained.
