...
void validate_audit_rule(const audit_rule& rule) {
    ...
    static const auto valid_categories = [] {
        std::set<sstring> result;
        for (auto cat : category_set::full()) {
            result.insert(category_to_string(cat));
        }
        return result;
    }();
    for (const auto& cat : rule.categories) {
        if (!valid_categories.contains(cat)) {
            throw audit_exception(fmt::format(
                "Bad configuration: invalid category '{}' in audit rule "
                "(must be one of: {})", cat, fmt::join(valid_categories, ", ")));
        }
    }
}
...
bool is_table_scoped_category(std::string_view category) {
    return category == "DML" || category == "DDL" || category == "QUERY";
}
...
bool matches_rule(const audit_rule& rule, std::string_view category,
    ...
    return matches_category(rule, category)
        && (is_table_scoped_category(category) ? matches_table(rule, keyspace, table) : true)
