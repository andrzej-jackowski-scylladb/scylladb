...
create_table_statement::do_prepare(data_dictionary::database db, prepare_context& ctx, cql_stats& stats, const cql_config& cfg) {
    ...
    throwing_assert(0 && "create_table_statement::do_prepare");
}
...
std::unique_ptr<prepared_statement> create_table_statement::raw_statement::do_prepare(data_dictionary::database db, prepare_context& ctx, cql_stats& stats, const cql_config& cfg) {
    // Column family name
