...
BOOST_AUTO_TEST_CASE(prepare_conjunction_empty) {
    ...
    BOOST_REQUIRE_EQUAL(prepare_expression(conj_empty, db, "test_ks", table_schema.get(), nullptr, internal_dialect()), expected);
    BOOST_REQUIRE_EQUAL(prepare_expression(conj_empty, db, "test_ks", table_schema.get(), make_receiver(boolean_type), internal_dialect()),
                        expected);
}
...
BOOST_AUTO_TEST_CASE(prepare_conjunction_empty_with_int_receiver) {
    ...
    BOOST_REQUIRE_THROW(prepare_expression(conj_empty, db, "test_ks", table_schema.get(), int_receiver, internal_dialect()),
                        exceptions::invalid_request_exception);
}
...
BOOST_AUTO_TEST_CASE(prepare_conjunction_one_untyped_const_false) {
    ...
    BOOST_REQUIRE_EQUAL(prepare_expression(conj_one, db, "test_ks", table_schema.get(), nullptr, internal_dialect()), expected);
    BOOST_REQUIRE_EQUAL(prepare_expression(conj_one, db, "test_ks", table_schema.get(), make_receiver(boolean_type), internal_dialect()),
                        expected);
}
...
BOOST_AUTO_TEST_CASE(prepare_conjunction_one_int_untyped_const_0) {
    ...
    BOOST_REQUIRE_THROW(prepare_expression(conj_one, db, "test_ks", table_schema.get(), nullptr, internal_dialect()),
                        exceptions::invalid_request_exception);
    BOOST_REQUIRE_THROW(prepare_expression(conj_one, db, "test_ks", table_schema.get(), make_receiver(boolean_type), internal_dialect()),
                        exceptions::invalid_request_exception);
}
...
BOOST_AUTO_TEST_CASE(prepare_conjunction_bools_and_one_int_untyped_const_0) {
    ...
    BOOST_REQUIRE_THROW(prepare_expression(conj, db, "test_ks", table_schema.get(), nullptr, internal_dialect()),
                        exceptions::invalid_request_exception);
    BOOST_REQUIRE_THROW(prepare_expression(conj, db, "test_ks", table_schema.get(), make_receiver(boolean_type), internal_dialect()),
                        exceptions::invalid_request_exception);
}
...
BOOST_AUTO_TEST_CASE(prepare_conjunction_and_of_ints_is_invalid) {
    ...
    BOOST_REQUIRE_THROW(prepare_expression(conj_one, db, "test_ks", table_schema.get(), nullptr, internal_dialect()),
                        exceptions::invalid_request_exception);
    BOOST_REQUIRE_THROW(prepare_expression(conj_one, db, "test_ks", table_schema.get(), make_receiver(int32_type), internal_dialect()),
                        exceptions::invalid_request_exception);
}
...
BOOST_AUTO_TEST_CASE(prepare_conjunction_many_elements) {
    ...
    BOOST_REQUIRE_EQUAL(prepare_expression(conj_many, db, "test_ks", table_schema.get(), nullptr, internal_dialect()), expected);
    BOOST_REQUIRE_EQUAL(prepare_expression(conj_many, db, "test_ks", table_schema.get(), make_receiver(boolean_type), internal_dialect()),
                        expected);

    // Integer receiver is rejected
    BOOST_REQUIRE_THROW(prepare_expression(conj_many, db, "test_ks", table_schema.get(), make_receiver(int32_type), internal_dialect()),
                        exceptions::invalid_request_exception);
}
...
void test_prepare_good_binary_operator(expression good_binop_unprepared,
    ...
    BOOST_REQUIRE_EQUAL(prepare_expression(good_binop_unprepared, db, "test_ks", table_schema.get(), nullptr, internal_dialect()),
                        expected_prepared);

    // Preparing with boolean receiver works as expected
    BOOST_REQUIRE_EQUAL(
        prepare_expression(good_binop_unprepared, db, "test_ks", table_schema.get(), make_receiver(boolean_type), internal_dialect()),
        expected_prepared);
...
void test_prepare_binary_operator_invalid_rhs_values(const expression& good_binop,
    ...
    for (const expression& invalid_rhs : invalid_rhs_vals) {
        ...
        BOOST_REQUIRE_THROW(prepare_expression(invalid_binop, db, "test_ks", table_schema.get(), nullptr, internal_dialect()),
                            exceptions::invalid_request_exception);
        BOOST_REQUIRE_THROW(
            prepare_expression(invalid_binop, db, "test_ks", table_schema.get(), make_receiver(boolean_type), internal_dialect()),
            exceptions::invalid_request_exception);
    }
}
...
BOOST_AUTO_TEST_CASE(test_aggregation_depth) {
    ...
    compared = prepare_expression(compared, db, "test_ks", schema.get(), nullptr, internal_dialect());
