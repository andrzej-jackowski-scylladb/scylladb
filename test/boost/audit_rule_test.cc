...
BOOST_AUTO_TEST_CASE(test_parse_audit_rules_json) {
    ...
    BOOST_CHECK_EQUAL(rules[0].categories.size(), 2u);
    ...
    for (const auto& json : {
        R"([{"categories":["DML"],"qualified_table_names":[],"roles":[]}])",
        R"([{"sinks":["table"],"qualified_table_names":[],"roles":[]}])",
        R"([{"sinks":["table"],"categories":["DML"],"roles":[]}])",
        R"([{"sinks":["table"],"categories":["DML"],"qualified_table_names":[]}])",
    }) {
        BOOST_CHECK_THROW(audit::parse_audit_rules_from_json(json), audit::audit_exception);
    }
...
BOOST_AUTO_TEST_CASE(test_validate_audit_rule) {
    ...
    for (const auto& category : {"INVALID", "dml", "D*", "", "[DA]*"}) {
        BOOST_CHECK_THROW(audit::validate_audit_rule(make_rule({"table"}, {category})), audit::audit_exception);
    }
}
...
BOOST_AUTO_TEST_CASE(test_config_audit_rules_rejects_invalid_values) {
    ...
    for (const auto& yaml : {
        ...
        BOOST_CHECK_THROW(cfg.read_from_yaml(yaml, throw_on_error), std::exception);
    }
...
BOOST_AUTO_TEST_CASE(test_matching_primitives) {
    ...
    BOOST_CHECK(audit::matches_category(category_rule, "DML"));
    BOOST_CHECK(audit::matches_category(category_rule, "AUTH"));
    BOOST_CHECK(!audit::matches_category(category_rule, "DDL"));
    BOOST_CHECK(!audit::matches_category(make_rule({"table"}), "DML"));
...
BOOST_AUTO_TEST_CASE(test_rule_matching_and_sinks) {
    ...
    BOOST_CHECK(audit::matches_rule(rule, "DML", "ks", "t1", "admin_read"));
    BOOST_CHECK(!audit::matches_rule(rule, "DML", "ks", "t2", "admin_read"));
    BOOST_CHECK(!audit::matches_rule(rule, "DML", "ks", "t1", "viewer"));

    BOOST_CHECK(audit::matches_rule(rule, "AUTH", "", "", "admin_read"));
    BOOST_CHECK(audit::matches_rule(rule, "AUTH", "wrong", "tbl", "admin_read"));

    BOOST_CHECK(audit::matches_rule(make_rule({"table"}, {"ADMIN"}, {"ks.t1"}, {"ops"}), "ADMIN", "any", "table", "ops"));
    BOOST_CHECK(audit::matches_rule(make_rule({"table"}, {"DCL"}, {"ks.t1"}, {"admin"}), "DCL", "", "", "admin"));
    BOOST_CHECK(!audit::matches_rule(make_rule({"table"}), "DML", "ks", "t", "admin"));
...
SEASTAR_THREAD_TEST_CASE(test_preprocessed_rules_match_fast_and_slow_paths) {
    ...
    BOOST_CHECK(rules.matching_sinks("DML", "ks", "t1", "admin_read").contains(audit::audit_sink::table));
    BOOST_CHECK(!rules.matching_sinks("DML", "ks", "t1", "viewer"));
    BOOST_CHECK(rules.matching_sinks("DML", "ks", "unknown", "admin_new").contains(audit::audit_sink::table));
    BOOST_CHECK(rules.matching_sinks("AUTH", "wrong", "tbl", "admin_read").contains(audit::audit_sink::table));

    auto ddl_sinks = rules.matching_sinks("DDL", "ks", "t2", "admin_read");
...
SEASTAR_THREAD_TEST_CASE(test_preprocessed_rules_update_known_entities) {
    ...
    BOOST_CHECK(rules.matching_sinks("DML", "ks", "t1", "admin"));
    ...
    BOOST_CHECK(!rules.matching_sinks("DML", "ks", "t1", "admin"));
    BOOST_CHECK(rules.matching_sinks("DDL", "ks", "t2", "viewer").contains(audit::audit_sink::syslog));
}

SEASTAR_THREAD_TEST_CASE(test_preprocessed_empty_rules_skip_cache) {
    audit::preprocessed_audit_rules rules;
    rules.replace_known_entities({"alice", "bob"}, {{"ks", "t1"}, {"ks", "t2"}}).get();
    BOOST_CHECK(!rules.matching_sinks("DML", "ks", "t1", "alice"));

    rules.add_known_role("charlie");
    rules.add_known_table("ks", "t3");
    BOOST_CHECK(!rules.matching_sinks("DML", "ks", "t3", "charlie"));
