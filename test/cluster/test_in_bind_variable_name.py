...
async def test_in_bind_variable_name_of_a_cached_statement(manager: ManagerClient):
    """
    The name is picked when the statement is prepared and the item is not part
    of the prepared statement cache key, so a statement that is already cached
    keeps the name it was prepared under. Applications are told to keep binding
    by the name they were given while the item is being rolled out, so the item
    has to be unable to rename a variable under them.

    The cache is per shard, hence the single-shard node: otherwise the second
    prepare could reach a shard that has never seen the statement, and would
    prepare it again, under the new name.
    """
    ...
    async with new_test_keyspace(manager,
        ...
        async with new_test_table(manager, keyspace, "p int, c int, PRIMARY KEY (p, c)") as table:
            ...
            logger.info("the item is false now, but the statement is already cached")
            ...
            logger.info("a statement prepared after the change does get the new name")
