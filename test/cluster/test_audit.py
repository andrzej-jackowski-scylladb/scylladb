...
class CQLAuditTester(AuditTester):
    ...
    async def _test_audit_rules_auth_with_empty_tables(self):
        ...
        logger.info("Reconnecting driver to generate a fresh AUTH entry")
        ...
        logger.info("Waiting for AUTH audit entry")
        def has_auth_entries():
            log_dict = self.get_audit_log_dict(session)
            for rows in log_dict.values():
                auth_entries = [r for r in rows if r.category == "AUTH"]
                if auth_entries:
                    return True
            return False
        wait_for(has_auth_entries, timeout=60)
