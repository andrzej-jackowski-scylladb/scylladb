...
# The name is chosen when the statement is prepared, and it is not part of the
# prepared statement cache key. The tests below therefore use their own table:
# reusing the query strings of the tests above would return the statement cached
# under the other spelling instead of preparing it again.
@pytest.fixture(scope="module")
def table3(cql, test_keyspace):
    with new_test_table(cql, test_keyspace, "p int, c int, PRIMARY KEY (p, c)") as table:
        yield table

def test_in_bind_marker_name_lowercase_operator(cql, table3, lowercase_in_bind_marker_name):
    prepared = cql.prepare(f"SELECT * FROM {table3} WHERE p = ? AND c IN ?")
    assert [col.name for col in prepared.column_metadata] == ['p', 'in(c)']
    x = unique_key_int()
    cql.execute(f'INSERT INTO {table3} (p,c) VALUES ({x},{x})')
    assert [(x,x)] == list(cql.execute(prepared, {'p': x, 'in(c)': [x, x+1]}))

# Same as test_in_bind_marker_name_lowercase_operator, but for NOT IN.
def test_not_in_bind_marker_name_lowercase_operator(cql, table3, scylla_only, lowercase_in_bind_marker_name):
    prepared = cql.prepare(f"SELECT * FROM {table3} WHERE p = ? AND c NOT IN ?")
    assert [col.name for col in prepared.column_metadata] == ['p', 'not in(c)']
    x = unique_key_int()
    cql.execute(f'INSERT INTO {table3} (p,c) VALUES ({x},{x})')
    cql.execute(f'INSERT INTO {table3} (p,c) VALUES ({x},{x+1})')
    assert [(x,x)] == list(cql.execute(prepared, {'p': x, 'not in(c)': [x+1]}))
...
# statement cache is that dropping a table evicts the statements prepared
# against it.
