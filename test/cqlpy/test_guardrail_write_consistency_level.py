...
def check_warned(cql, test_keyspace, query, cl=ConsistencyLevel.ONE, config_value=None):
    with config_value_context(cql, "write_consistency_levels_warned", config_value or ConsistencyLevel.value_to_name[cl]):
        ...
        stmt = SimpleStatement(query.format(table=test_table), consistency_level=cl)
        ret = cql.execute(stmt)
...
def check_disallowed(cql, test_keyspace, query, cl=ConsistencyLevel.ONE, config_value=None):
...
def check_guardrailed(cql, test_keyspace, query):
    check_warned(cql, test_keyspace, query)
    check_disallowed(cql, test_keyspace, query)

def test_write_cl_insert(cql, test_keyspace, test_table):
    check_guardrailed(cql, test_keyspace, f"INSERT INTO {test_table} (pk, v) VALUES (1, 1)")

def test_write_cl_update(cql, test_keyspace, test_table):
    check_guardrailed(cql, test_keyspace, f"UPDATE {test_table} SET v = 2 WHERE pk = 1")

def test_write_cl_delete(cql, test_keyspace, test_table):
    check_guardrailed(cql, test_keyspace, f"DELETE FROM {test_table} WHERE pk = 1")

def test_write_cl_conditional_insert(cql, test_keyspace, test_table):
    check_guardrailed(cql, test_keyspace, f"INSERT INTO {test_table} (pk, v) VALUES (5, 5) IF NOT EXISTS")

def test_write_cl_counter(cql, test_keyspace, counter_table):
    check_guardrailed(cql, test_keyspace, f"UPDATE {counter_table} SET c = c + 1 WHERE pk = 1")

def test_write_cl_select_not_affected(cql, test_keyspace, test_table):
    check_no_warning(cql, test_keyspace, f"SELECT * FROM {test_table} WHERE pk = 1", cl=ConsistencyLevel.QUORUM, warned="QUORUM", is_write=False)

def test_write_cl_logged_batch(cql, test_keyspace, test_table):
    check_guardrailed(cql, test_keyspace, f"""
        BEGIN BATCH
...
def test_write_cl_unlogged_batch(cql, test_keyspace, test_table):
    check_guardrailed(cql, test_keyspace, f"""
...
def test_write_cl_conditional_batch(cql, test_keyspace, test_table):
    check_guardrailed(cql, test_keyspace, f"""
        BEGIN BATCH
...
def test_write_cl_no_warning(cql, test_keyspace, test_table):
    check_no_warning(cql, test_keyspace, f"INSERT INTO {test_table} (pk, v) VALUES (4, 4)", cl=ConsistencyLevel.ALL, warned="QUORUM", disallowed="ANY")

def test_write_cl_empty_config(cql, test_keyspace, test_table):
    check_no_warning(cql, test_keyspace, f"INSERT INTO {test_table} (pk, v) VALUES (4, 4)", warned="", disallowed="")
...
def test_write_cl_multiple_warned_levels(cql, test_keyspace, test_table):
    ...
    config = "ONE,QUORUM"
    check_warned(cql, test_keyspace, query, cl=ConsistencyLevel.ONE, config_value=config)
    check_warned(cql, test_keyspace, query, cl=ConsistencyLevel.QUORUM, config_value=config)
    check_no_warning(cql, test_keyspace, query, cl=ConsistencyLevel.LOCAL_ONE, warned=config)

def test_write_cl_multiple_disallowed_levels(cql, test_keyspace, test_table):
    ...
    query = f"INSERT INTO {test_table} (pk, v) VALUES (1, 1)"
    config = "ANY,ALL"
    check_disallowed(cql, test_keyspace, query, cl=ConsistencyLevel.ANY, config_value=config)
    check_disallowed(cql, test_keyspace, query, cl=ConsistencyLevel.ALL, config_value=config)
    check_no_warning(cql, test_keyspace, query, cl=ConsistencyLevel.LOCAL_ONE, disallowed=config)
