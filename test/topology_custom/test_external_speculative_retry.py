...
@pytest.mark.parametrize("query_timeout, external_timeout, expected_error", [
    (1800, 30000, "Operation failed for ks.test_table"),
    (30, 1800000, "Operation timed out")
])
@pytest.mark.asyncio
async def test_external_speculative_retry(request, manager: ManagerClient, query_timeout, external_timeout, expected_error):
    ...
    for i in range(200):
        await cql.run_async(f"insert into ks.test_table (key, value) VALUES ({i}, {i})")
    ...
    query_future = cql.run_async(f"select COUNT(*) from ks.test_table BYPASS CACHE USING TIMEOUT {query_timeout}s", timeout=3600)
